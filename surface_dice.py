...
class SurfaceDiceMetric:
    def __init__(self, n_batches, device):
        ...
        self.device = device
        self.area_table = torch.from_numpy(
            self.create_table_neighbour_code_to_surface_area((1, 1, 1))
        ).to(self.device)
        self.batch_idx = 0
        self.numerator = 0
        self.denominator = 0
    ...
    def compute_surface_area(self, surface):
        ...
        weight = (2 ** torch.arange(8, device=self.device, dtype=torch.float32)).view(
            1, 1, 2, 2, 2
        )
        cubes_byte = torch.nn.functional.conv3d(
            surface, weight, padding=(0, 1, 1)
        ).flatten()
