...
class Patcher:
    ...
    def extract_patches(self, x):
        ...
        patches = patches.permute(0, 2, 3, 1, 4, 5).contiguous()   # (B, h_steps, w_steps, C, patch_size, patch_size)                          
        patches = patches.view(B, -1, C, self.patch_size, self.patch_size)  # (B, n_patches, C, patch_size, patch_size)                         
        return patches
    ...
    def merge_patches(self, patches):
        ...
        x = patches.permute(0, 2, 3, 4, 1).view(B, C * self.patch_size * self.patch_size, N)  
